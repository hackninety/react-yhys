...
基于邵雍《皇极经世》体系与黄畿《皇极经世书传》注解构建的 React 日历应用，实现了元、会、运、世、年五级时间单位的可视化展示，并整合了律吕音高系统与声音唱和音韵矩阵。
...
| **卦象派生** | “以大统小”的层级结构——详解元会运世岁五级卦象之间的派生关系 | 会→运→世→岁→月→日 卦算法 |
...
│   ├── components/         # React 组件
│   │   ├── Calendar.tsx    # 主日历组件
│   │   ├── DateDetailModal.tsx  # 日期详情弹窗（八字/卦象/律吕/唱和）
│   │   └── ChangheMatrix.tsx   # 全屏声音唱和矩阵图
│   ├── data/               # 数据文件
│   │   ├── hexagrams64.ts  # 64卦象数据与卦算法
